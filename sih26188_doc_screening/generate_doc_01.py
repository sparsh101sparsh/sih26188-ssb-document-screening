...
doc_path = "/Users/iamsparsh00321/Documents/antigravity/vibrant-rutherford/sih26188_doc_screening/docs/01_OCR_AND_MRZ_MODULE.md"

content = r"""# Module 01: Multilingual Document OCR, Key Information Extraction & MRZ/Barcode Cryptographic Verification
...
- **Indian Passports (ICAO Doc 9303 TD3)**: 2 lines $\times$ 44 characters OCR-B font.
...
2. **Tier-2 Quality-Gate Path (Qwen2.5-VL-3B)**: If PP-OCRv4 encounters heavy physical abrasion, handwritten annotations, or faint dot-matrix printing resulting in $C_{\text{avg}} < 0.82$ or a regex validation failure on the document number, the image is routed to Qwen2.5-VL-3B-Instruct running in INT4 AWQ precision on GPU.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **OmniMRZ** | Morphological + DBNet | PP-OCRv4 (OCR-B Tuned) | **TD1, TD2, TD3, MRVA/B** | Complete Native Engine | **65 ms** | **High (up to 35° tilt)** | **🏆 Winner** |
...
#### Checksum Equation:
For a character sequence $S = s_1 s_2 \dots s_k$ and repeating weights $W = [7, 3, 1, 7, 3, 1, \dots]$:

$$\text{CheckDigit}(S) = \left( \sum_{i=1}^k \text{Val}(s_i) \times W_{(i-1) \pmod 3} \right) \pmod{10}$$

#### Document Formats:
- **TD1 (National ID Cards / Border Passes)**: 3 lines $\times$ 30 characters ($3 \times 30 = 90$ chars).
- **TD2 (Visas / Official Travel Passes)**: 2 lines $\times$ 36 characters ($2 \times 36 = 72$ chars).
- **TD3 (Standard International Passports)**: 2 lines $\times$ 44 characters ($2 \times 44 = 88$ chars).
...
```python
"""
SIH26188: Complete ICAO Doc 9303 Modulo-10 7-3-1 Checksum Engine
...
class ICAO9303Validator:
    ...
    def parse_td3_passport(cls, line1: str, line2: str) -> Dict[str, Any]:
        """
        Parses and cryptographically validates an ICAO Doc 9303 TD3 2-line x 44-char MRZ.
        """
...
```python
"""
SIH26188: Aadhaar Secure QR Offline PKI Verifier & JP2000 Facial Photo Extractor
"""
import zlib
import cv2
import numpy as np
...
class AadhaarSecureQRVerifier:
    ...
    def decode_and_verify(self, image_path_or_ndarray: Any) -> Dict[str, Any]:
        ...
        # Step 1: Extract Raw Binary Matrix via zxing-cpp
        ...
        # Step 5: Parse Demographic Fields and Extract JP2000 Photo
...
When an adversary alters printed text on a document (e.g., physically modifying Date of Birth or Name on an Aadhaar or Passport), they cannot modify the underlying cryptographic signature or ICAO check digits without corrupting verification.
...
print(f"Doc 01 generated successfully at: {doc_path} (Length: {len(content)} characters)")
